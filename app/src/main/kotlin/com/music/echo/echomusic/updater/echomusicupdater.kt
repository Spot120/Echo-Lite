...
suspend fun checkForUpdate(
    ...
    // Echo Lite intentionally never checks for or offers updates. The upstream release is a
    // different build of the same app (same package name), so installing it here would silently
    // replace this fork -- including every custom feature and setting added on top of it -- with
    // stock Echo Music. Reporting "up to date" (isAvailable = false) keeps any update UI that
    // reads this callback in a normal, quiet state instead of showing an error.

...
class AudioExportService : Service() {
    ...
    // Multiple exports (e.g. "export all downloads") can be in flight at once since each
    // onStartCommand launches its own coroutine on the same service instance. Only stop the
    // service once every in-flight export has actually finished, otherwise stopSelf() would
    // cancel serviceScope and kill sibling exports that are still running.
    private val activeExportCount = AtomicInteger(0)

    // Bulk exports (e.g. "export all downloads") queue every song at once; without a cap this
    // would spin up one FFmpeg process per song simultaneously, which is unnecessarily heavy on
    // low-end devices. Two at a time keeps things responsive without serializing everything.
    private val conversionSemaphore = Semaphore(permits = 2)
    ...
    private suspend fun exportSong(
        ...
        try {
            ...
            // Only shut the service down once every concurrently queued export has completed.
            if (activeExportCount.decrementAndGet() <= 0) {
                stopSelf()
            }
        }
    }
